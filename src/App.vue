...
<template>
  <!-- usar directiva v-bind:class para asinar clase class si isDark en el store es true(listo) -->
  <div class="wrapper transition ease-linear" :class="{ dark: theme.isDark }">
    <div class="btn-wrapper">
      <div class="toggle-btn flex items-center justify-center w-full my-4">
        <label for="toggle" class="flex items-center justify-center cursor-pointer">
          <div class="relative">
            <!-- usar directiva @click para ejecutar el metodo para cambiar de dark a light o viceversa(listo)-->
            <input type="checkbox" id="toggle" class="sr-only" />
            <div class="block bg-gray-600 w-14 h-8 rounded-full" @click="theme.toggleTheme"></div>

            <div
              class="dot absolute left-1 top-1 bg-black w-6 h-6 flex items-center justify-center rounded-full transition" @click="theme.toggleTheme" >
              <!-- usar directiva v-if  para mostrar el icono de luna o sol usando isDark como referencia (listo)-->
          ...
          <!-- cambiar usar v-bind:class y atributo mode del ThemeState para cambiar el texto (listo)-->
          <div class="label-text ml-2 font-medium" :class="{ dark: theme.isDark }">
            {{ theme.isDark ? 'Modo Oscuro' : 'Modo Claro' }} </div>
        </label>
      </div>
    </div>
    <!-- usar directiva v-bind:class para asinar clase class si isDark en el store es true (listo) -->
    <div class="img min-h-screen flex flex-col items-center transition" :class="{ dark: theme.isDark }"></div>
