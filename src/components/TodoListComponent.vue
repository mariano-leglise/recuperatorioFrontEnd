...
<template>
    <!-- div: usar v-bind:class para cambiar a modo oscuro(listo) -->
    <div class="list-wrapper max-w rounded overflow-y-auto shadow-lg mt-10 p-4 transition ease-linear" :class="{ dark: theme.isDark }">
        ...
        <!-- envoltura de la tarea, insertar v-for aca!(listo) -->
        <div v-for="task in taskStore.data" :key="task.id" class="wrapper relative group border-black my-2 transition ease-linear">
            <form v-on:submit.prevent>
                    ...
                    <div class="relative">
                        <input
                        type="ckeckbox"
                        @click="task.status = !task.status" class="form-checkbox border rounded-full focus:ouline-none h-6 w-6 cursor-pointer transition ease-linear"
                        />
                        <!-- usar directiva v-if para mostrar si la tarjeta esta completada -->
                        <CompletedIcon v-if="task.status" class="h-100 w-100 absolute left-0 top-0 text-green-600"/>
                    </div>
                </div>
                ...
                <input
                    ...
                    type="text"
                    v-model="task.tarea" class="tarea sm:text-base overflow-ellipsis w-full disabled:bg-white focus:outline-none py-4 sm:py-4.5 pr-8 pl-14 sm:pl-16 cursor-pointer transition ease-linear"
                    />
                <div class="btns absolute right-0 top-0 py-2 sm:py-2.5 px-2 w-20 h-14 flex justify-around cursor-default" >

                    <!-- usar @click y llamar a funcion para borrar tarea(listo) -->
                    <button @click="taskStore.removeTask(task)" class="p-1 cursor-pointer"><TrashIcon class="w-6 h-6 hover:text-red-500 "/></button>
                </div>

                <!-- indicador de tarea terminada, usar v-if segun corresponda -->
                <span v-if="task.status" class="badge absolute right-10 inline-block bg-green-200 text-teal-800 text-xs px-2 rounded-full uppercase font-semibold tracking-wide">Completo</span>
